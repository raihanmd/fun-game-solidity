...
contract Raffle is VRFConsumerBaseV2Plus, AutomationCompatibleInterface {
    ...
    /// @notice Emitted requesting a VRF random number
    /// @param reqId The ID of the VRF request
    event Raffle__RequestedRaffleWinner(uint256 indexed reqId);
    ...
    enum RaffleState {
        OPEN,
        /// @dev Raffle is open for entries (0)
        CALCULATING
    }
    /// @dev Raffle is calculating winner using VRF (1)
    ...
    function pickWinner() internal {
        ...
        // TODO redundant, it also emitted in VRFCoordinator
        emit Raffle__RequestedRaffleWinner(reqId);
    }
    ...
    /**
     * @notice Returns all player by index in the current raffle round
